...
class PictureViewer(object):
    ...
    def parsr_infor_from_text(self,text):
        ...
        if( int(float(text[1]) )==-1):
            output_dict["relpath"]=text[0]
            output_dict["abspath"]=os.path.join(self.label_file_dir,text[0])
            output_dict["flag"]=0
    ...
    def is_empty(self,index):
        return not self.all_data_list[self.display_data_index[index]]["flag"]
                        ...
                        if(key=="relpath"):
                            ...
                            if(key!='flag'):
                                ...
                                if(rule_index==len(init.glb_output_rule_list) ):
                                    ...
                                    output+="\n"
                f.writelines( output )
        print "save label file sucessful"
        return 1
...
if __name__=="__main__":
    test=PictureViewer("testImageList.txt","BBox_5PointsLandmark.txt",0)
